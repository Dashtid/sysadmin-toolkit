...
- 2026-06-09: `test(development): behavioral coverage for Manage-WSL` - 32 new tests, +2.86 pp overall coverage (18.08% -> 20.94%). Refactor: renamed `Main` to `Invoke-WslManager` with SupportsShouldProcess, added testability guard, gave `Set-WslConfiguration` a `-ConfigPath` parameter so config writes can be sandboxed to `$TestDrive` instead of clobbering `~/.wslconfig`. Highest-risk script in the set (risk=5: distro export/import, --unregister can permanently delete user data) is now covered. No production bugs found.
- 2026-06-09: `test(development): behavioral coverage for Manage-Docker` (commit 9205a31) - 29 new tests, +2.17 pp overall coverage (15.91% -> 18.08%). Refactor: renamed `Main` to `Invoke-DockerManager` (matches Invoke-X pattern), added testability guard, gave `Test-DockerDesktopInstalled` / `Start-DockerDesktop` a `-Path` parameter, renamed `Pull-DockerImage` to `Invoke-DockerImagePull` (approved verb), and renamed `$args` to `$dockerArgs` in two helpers to avoid shadowing the PowerShell automatic variable. No production bugs found this round -- script was already well-shaped.
